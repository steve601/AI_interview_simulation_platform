...
interface InterviewContextType {
  ...
  setJobDescription: React.Dispatch<
    React.SetStateAction<JobDescriptionInfo>
  >;

  // Backend analysis results
  cvAnalysis: InterviewPlanBackend['cv_analysis'] | null;
  setCvAnalysis: (
    v: InterviewPlanBackend['cv_analysis'] | null
  ) => void;

  jdAnalysis: InterviewPlanBackend['jd_analysis'] | null;
  setJdAnalysis: (
    v: InterviewPlanBackend['jd_analysis'] | null
  ) => void;

  gapAnalysis: InterviewPlanBackend['gap_analysis'] | null;
  setGapAnalysis: (
    v: InterviewPlanBackend['gap_analysis'] | null
  ) => void;

  interviewPlan:
    | InterviewPlanBackend['interview_plan']
    | null;

  setInterviewPlan: (
    v: InterviewPlanBackend['interview_plan'] | null
  ) => void;

  // Interview
  interviewMessages: InterviewMessage[];
  setInterviewMessages: React.Dispatch<
  ...
  setFinalReport: (
    v: FeedbackResponseBackend | null
  ) => void;
  ...
  setRoundProgress: React.Dispatch<
    React.SetStateAction<RoundProgressState>
  >;
...
const createEmptyCandidate = (): CandidateInfo => ({
  candidateName: '',
  email: '',
  yearsOfExperience: 0,
  extractedSkills: [],
...
const createEmptyJobDescription = (): JobDescriptionInfo => ({
  jobTitle: '',
  company: '',
  department: '',
  location: '',
...
const createDefaultRoundProgress =
  (): RoundProgressState => ({
    behavioral: {
      completed: false,
      answeredCount: 0,
...
export const InterviewProvider: React.FC<{
  ...
  const [jobDescription, setJobDescription] =
    useState<JobDescriptionInfo>(
      createEmptyJobDescription()
    );

  const [cvAnalysis, setCvAnalysis] =
    useState<
      InterviewPlanBackend['cv_analysis'] | null
    >(null);

  const [jdAnalysis, setJdAnalysis] =
    useState<
      InterviewPlanBackend['jd_analysis'] | null
    >(null);

  const [gapAnalysis, setGapAnalysis] =
    useState<
      InterviewPlanBackend['gap_analysis'] | null
    >(null);

  const [interviewPlan, setInterviewPlan] =
    useState<
      InterviewPlanBackend['interview_plan'] | null
    >(null);

  const [interviewMessages, setInterviewMessages] =
    useState<InterviewMessage[]>([]);

  const [currentRound, setCurrentRound] =
    useState<RoundType>('behavioral');

  const [isLoadingInterview, setIsLoadingInterview] =
    useState(false);
  ...
  const [finalReport, setFinalReport] =
    useState<FeedbackResponseBackend | null>(null);
  ...
  const resetSession = () => {
    ...
    addToast({
      ...
      title: 'Session Reset',
      message:
        'Ready for a new interview session.',
    });
  };
  ...
  const loadSampleSession = () => {
    setCandidate({
      ...
      extractedText:
        'Senior frontend engineer with experience leading platform initiatives and mentoring engineers.',
    });
    ...
    setJobDescription({
      ...
      fullText:
        'Senior Frontend Engineer responsible for building reliable, performant user experiences across enterprise products.',
    });
    ...
    addToast({
      type: 'success',
      title: 'Sample session loaded',
      message:
        'Demo candidate and role data are ready.',
    });
  };
